...
class Node:
    ...
    def send_broadcast_block(self, target_block, from_nodeid):
        if self.if_blockinchain(target_block) == 0:
            ffast = 0
            fslow = 0
            if self.nodeid == target_block.block_nodeid and from_nodeid == self.nodeid:        #my own block on me
                for i in reversed(self.peers):
            ...
            elif self.nodeid != target_block.block_nodeid:
                for i in self.peers:
                    if i.nodeid == target_block.block_nodeid or i.nodeid == self.nodeid or i.nodeid == from_nodeid:
                        pass
                    else:
                        if self.nature == 1 and i.nature == 1:
                            if ffast == 0:
    ...
    def recv_broadcast_block(self, target_block, sender_nodeid):
        if self.if_blockinchain(target_block) == 0 :          #if the block at the end of longest chain is the same as prev. block of the new block
            ...
            if target_block.block_nodeid == self.nodeid and final_last.blockid == target_block.prev_block:
                print "Same", target_block.block_nodeid, self.nodeid
            #target_block.block_nodeid == self.nodeid or sender_nodeid == self.nodeid or sender_nodeid == target_block.block_nodeid
            if target_block.block_nodeid == self.nodeid:       #if i had only generated this block
                print "Just me"
                pass
            else:
                #print "Received", target_block.blockid, target_block.block_nodeid, sender_nodeid, self.nodeid
                self.block_rcvd = 1
                for n in self.peers:
                    if n.nodeid == sender_nodeid or n.nodeid == self.nodeid or n.nodeid == target_block.block_nodeid:
                        pass
                    else:
                        n.send_broadcast_block(target_block, self.nodeid)
